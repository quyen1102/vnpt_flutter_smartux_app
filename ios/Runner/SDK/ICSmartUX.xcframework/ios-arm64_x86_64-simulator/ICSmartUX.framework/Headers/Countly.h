...
/**
 * Returns current metrics being used for tracking.
 * @discussion Metrics can be used for user flow.
 */
- (NSString *)metrics;
 ...
 * @param sum Sum of any specific value for event
 * @param duration Duration of event in seconds
 * @param trigger Trigger to hook into event
 */
- (void)recordEvent:(NSString *)key segmentation:(NSDictionary *)segmentation count:(NSUInteger)count sum:(double)sum duration:(NSTimeInterval)duration trigger:(NSString *)trigger;
